---
description: 成为一个验证程序并帮助保护伦理2，一个验证的股份区块链。 任何拥有32 ETH的人都可以加入。
---

# 指南\| 如何在ETH2主网放样上设置验证器

{% hint style="success" %}
截止2020年12月5日，本指南已更新为 **主网。**
{% endhint %}

#### 对于测试网指南， [请点击这里](guide-or-how-to-setup-a-validator-on-eth2-testnet.md)。
...
请 [来看看我们的](https://gitcoin.co/grants/1653/eth2-staking-guides-by-coincashew)。 谢谢!

{% embed url="https://gitcoin.co/grants/1653/eth2-staking-guides-by-coincashew" caption="" %}

## 🏁 0. 必备条件

### 操作技能 eth2 validator and 信标节点
...
{% hint style="success" %}
✨ **Pro Validator 提示**: 高度推荐您以一个全新的 OS、VM 和/或机器实例开始。 避免头痛为您的验证器重新使用测试网键、钱包或数据库。
{% endhint %}
...
如果您需要安装 Ubuntu 服务器，请参阅：[https://ubuntu.com/tutorials/install-ubuntu-server\#1-overview](https://ubuntu.com/tutorials/install-ubuntu-server#1-overview)

或者Ubuntu 桌面，

### 🎭 设置Metamask

如果您需要安装Metamask，请参阅：[https://www.coincashew.com/wallets/browser-wallets/metamask-ethereum](https://www.coincashew.com/wallets/browser-wallets/metamask-ethereum)
...
1. 安装依赖，ethereum基金会存款工具和生成两套密钥。

{% hint style="info" %}
每个验证器将有两组密钥对。 一个 **签名密钥** 和 **提款密钥。** 这些密钥来自单词句。 [了解更多关于密钥的信息](https://blog.ethereum.org/2020/05/21/keys/)
{% endhint %}

您可以选择下载预构建的 [ethereum基础存款工具](https://github.com/ethereum/eth2.0-deposit-cli) 或从源代码构建它。
...
2. 跳过您刚刚完成的步骤时，请在 [https://launchpad.ethereum.org/](https://launchpad.ethereum.org/) 上按照这些步骤。 研究eth2阶段0概览材料。 了解eth2是成功的关键！
...
{% hint style="info" %}
**OpenEthereum** - **目标是使用** Rust 编程语言\*\* 成为最快、最轻松和最安全的 Ethereum 客户端。 OpenEthereum 是在 GPLv3 下许可的，可以用于满足您所有的以太坊需要。
{% endhint %}
...
查看最新版本： [https://github.com/openethereum/openethereum/releases](https://github.com/openethereum/openethereum/releases)
...
运行下面的操作来创建一个 **单元文件** 来定义您的 `eth1.service` 配置。
...
查看最新版本： [https://github.com/hyperledger/besu/releases](https://github.com/hyperledger/besu/releases)

文件可以从 [https://dl.bintray.com/hyperledger-org/besu-repo](https://dl.bintray.com/hyperledger-org/besu-repo) 下载。
...
1. 注册免费帐户。
2. 确认您的电子邮件地址。
3. 访问您的仪表板 [https://infura.io/dashboard](https://infura.io/dashboard)
4. 创建一个项目，给它一个名称。
5. 选择 **Mainnet** 作为ENDPOINT
6. 按照下面您的 eth2 客户端的特定配置。

{% hint style="success" %}
或者，在 [https://ethereumnodes.com/](https://ethereumnodes.com/) 上使用一个免费的Ethereum 节点
{% endhint %}
...
{% hint style="info" %}
[Lighthouse](https://github.com/sigp/lighthouse) 是一个 Eth2.0 客户端，对速度和安全性的高度重视。 背后的团队， [Sigma Prime](https://sigmaprime.io/)， 这是一家信息安全和软件工程公司，它向灯塔连同埃瑟姆基金会共识和个人提供了资金。 灯塔是用Rust构建的，提供给Apache 2.0 许可的。
{% endhint %}
...
{% hint style="info" %}
 **端口转发提示：** 您需要转发并打开端口到您的验证器。 验证它使用 [https://www.yougetsignal.com/tools/open-ports/](https://www.yougetsignal.com/tools/open-ports/) 或 [https://canyousee.org/](https://canyouseeme.org/)。
{% endhint %}
...
{% hint style="info" %}
🔥 **Lighthouse Pro 提示:** 在 **ExecStart** 行上，添加了 `--eth1-endpoint` 标记允许多余的eth1节点。 用逗号分隔。 请确保端点不会以尾随斜杠或`/` 移除它。
...
journalctl --unit=beacon-chain --since='2020-12-01 00:00:00' --until='2020-12-02 12:00'
```
...
{% hint style="warning" %}
Nimbus只支持 ETH1 节点的 websocket 连接\("ws://" 和 "wss://"\"\)。 Geth, OpenEthereum, Infura and Chainstack ETH1 nodes are verified compatible.
{% endhint %}
...
{% hint style="info" %}
✨ **端口转发提示:** 你需要转发并打开端口到你的验证器。 验证它使用 [https://www.yougetsignal.com/tools/open-ports/](https://www.yougetsignal.com/tools/open-ports/) 或 [https://canyousee.org/](https://canyouseeme.org/)。
{% endhint %}
...
MY_GRAFFITI='<my POAP string or message>
# 示例
# MY_GRAFFITI='poapAAAAAAACGatUA1bLuDnL4FMD13BfoD'
# MY_GRAFFITI='eth2 rulz!'
```
...
{% tab title="Lodestar" %}
{% hint style="info" %}
**Lodestar is a Typescript implementation** of the official [Ethereum 2.0 specification](https://github.com/ethereum/eth2.0-specs) by the [ChainSafe.io](https://lodestar.chainsafe.io/) team. 除了信标链客户外，该小组还在22个软件包和图书馆工作。 完整的列表可以在这里找到 。 末尾。 Lodestar小组正在领导Eth2空间的轻型客户研究和开发，并为此目的从EF和Moloch DAO获得资金。
{% endhint %}
...
#### :whit\_heavy\_check\_mark: 检查验证器是否激活
...
#### :countrockwise\_arrows\_buton: 重新启动验证器
...
{% tab title="Prysm" %}
![](../../../.gitbook/assets/eth2-grafana-dashboard-prysm.jpg)
...
1. 访问 [https://beaconcha.in/](https://beaconcha.in/)
2. 注册一个帐户。
3. 验证您的 **电子邮件**
4. 搜索您的 **验证器的公开地址**
...
### 🌊 6.4 由 Google Cloud 使用 Uptime 检查
...
{% hint style="success" %}
🎉恭喜您设置了验证器！你很高兴继续使用 eth2.0 。

你觉得我们的指南有用吗？ 让我们知道一个小，我们会不断更新它。
...
使用 eth2deposit-cli 工具来恢复您的 `validator_key` 以确保您可以重新生成相同的 eth2 键对。
...
{% hint style="info" %}
当两个密钥库文件中的pubkey相同时，这意味着您的助记词确实是正确的。 其他字段因为打折而不同。
{% endhint %}

### 🤖8.3 添加额外的验证器

使用 eth2deposit-cli 工具，您可以通过创建新的存款数据文件和 `validator_keys` 来添加更多的验证器
...
```bash
cd $HOME/eth2deposit-cli
./deposit.sh existing-mnemonic --validator_start_index 3 --num_validators 5 --chain mainne
```
...
####   🔥 8.4.6 验证功能 <a id="8-4-5-start-new-validator-and-new-beacon-chain"></a>
...
**源引用**: [https://askubuntu.com/questions/1106795/ubuntu-server-18-04-lvm-out-of-space-with-improper-default-partitioning](https://askubuntu.com/questions/1106795/ubuntu-server-18-04-lvm-out-of-space-with-improper-default-partitioning)
...
#Slash protection db
~/.lighthouse/mainnet/validators/slash_protection.sqlite
```
{% endtab %}
...
```bash
# 编辑信标链单元文件
nano /etc/systemd/beacon-chain.service
# 添加--eth1-endpoint 参数
# 示例
...
### 📦 8.10 更新一个 ETH1 节点 - Geth / OpenEthereum / Besu / Nethermind
...
{% tabs %}
{% tab title="Geth" %}
在 [https://github.com/ethereum/go-ethereum/releases](https://github.com/ethereum/go-ethereum/releases) 查看最新版本说明
